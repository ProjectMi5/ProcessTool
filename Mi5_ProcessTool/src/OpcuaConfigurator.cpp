...
OpcuaConfigurator::OpcuaConfigurator()
{
}
...
UaString OpcuaConfigurator::getServerUrl() const
{
    return m_serverUrl;
}

UaString OpcuaConfigurator::getDiscoveryUrl() const
{
    return m_discoveryUrl;
}

UaString OpcuaConfigurator::getApplicationName() const
{
    return m_applicationName;
}

OpcUa_Boolean OpcuaConfigurator::getAutomaticReconnect() const
{
    return m_bAutomaticReconnect;
}

OpcUa_Boolean OpcuaConfigurator::getRetryInitialConnect() const
{
    return m_bRetryInitialConnect;
}

UaNodeIdArray OpcuaConfigurator::getNodesToRead() const
{
    return m_nodesToRead;
}

UaNodeIdArray OpcuaConfigurator::getNodesToWrite() const
{
    return m_nodesToWrite;
}

UaNodeIdArray OpcuaConfigurator::getNodesToSubscribe() const
{
    return m_nodesToSubscribe;
}

UaVariantArray OpcuaConfigurator::getWriteValues() const
{
    return m_writeValues;
}

UaStatus OpcuaConfigurator::loadConfiguration(const UaString& sConfigurationFile)
{
    UaStatus result;
    UaVariant value;
    UaString sTempKey;
    ...
    pSettings->beginGroup("MI5_Config");
    ...
    // Server URLs
    value = pSettings->value("DiscoveryURL", UaString("opc.tcp://localhost:1337"));
    m_discoveryUrl = value.toString();
    value = pSettings->value("ServerUrl", UaString("opc.tcp://localhost:1337"));
    ...
    // Read NodeIds to use for subscribing
    m_nodesToSubscribe.clear();
    pSettings->beginGroup("NodesToSubscribe");
    value = pSettings->value("size", (OpcUa_UInt32)0);
    value.toUInt32(size);
    m_nodesToSubscribe.resize(size);

    for (i = 0; i < size; i++)
    {
        sTempKey = UaString("Variable0%1").arg((int)i);
        value = pSettings->value(sTempKey.toUtf16(), UaString(""));
        UaNodeId::fromXmlString(value.toString()).copyTo(&m_nodesToSubscribe[i]);
    }

    pSettings->endGroup();  // NodesToSubscribe
...
UaStatus OpcuaConfigurator::updateNamespaceIndexes(const UaStringArray& namespaceArray)
{
    UaStatus result;
    OpcUa_UInt32 i, j;
    OpcUa_UInt32 size;
    ...
    // replace namespace index in NodeIds // What is done here?
    // NodesToRead
